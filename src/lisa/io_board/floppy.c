...
#else
    void append_floppy_log(const char *s) {s=NULL;}
#endif
       ...
       DEBUG_LOG(0,"%s: cpuclock:%016llx, current sector:%ld,(side/track/sect:%d:%d:%d)",
        ...
        switch ( queuedfn)
        {
         ...
         case FLOP_CMD_WRITE : append_floppy_log("completing pending WRITE IRQ in RWTS");
                                   {
                                       DC42ImageType *F=FQ; // F is used by RWTS_IRQ_SIGNAL
                                       if (FQ && FQ->RAM)
                                       {
                                           do_floppy_write(FQ);
                                       }
...
//insert_floppy
int floppy_insert(const char *Image)     // emulator should call this when user decides to open disk image...
{
    DC42ImageType *F;
    int err=0;
    int floppynum=1;
